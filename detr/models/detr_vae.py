...
#from pointclouds.pointnet import PointNet,SparsePointNet, MinkowskiPointNet
...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, point_cloud_model, transformer, encoder, state_dim, num_queries, use_cameras, use_point_clouds):
        ...
        self.encoder_action_proj = nn.Linear(
            self.state_dim, hidden_dim)  # project action to embedding
        self.encoder_joint_proj = nn.Linear(
            self.state_dim, hidden_dim)  # project qpos to embedding
        # project hidden state to latent std, var
        self.latent_proj = nn.Linear(hidden_dim, self.latent_dim*2)
        self.register_buffer('pos_table', get_sinusoid_encoding_table(
            1+1+num_queries, hidden_dim))  # [CLS], qpos, a_seq
        ...
        # Point cloud model
        self.point_cloud_model = point_cloud_model

    def forward(self, qpos, image, point_cloud, point_cloud_len, env_state, actions=None, is_pad=None):
        """
        qpos: batch, qpos_dim
        ...
        bs, n_images, *_ = image.shape
        _, n_point_clouds, *_ = point_cloud.shape
        ...
        if self.backbones is not None:
            ...
            if self.use_point_clouds:
                for cam_id in range(n_point_clouds):
                    batch_cam_features = []
                    batch_cam_pos = []
                    for bi in range(bs):
                        point_cloud_data = point_cloud[bi,
                                                       cam_id][:point_cloud_len[bi, cam_id]]
                        point_cloud_data = point_cloud_data.unsqueeze(0)
                        features, pos = self.point_cloud_model(
                            point_cloud_data)
                        features = features[0]  # take the last layer feature
                        # pos = pos[0]
                        batch_cam_features.append(features)
                        batch_cam_pos.append(pos)

                    # Append all the batch features and pos into a single tensor and append
                    features = torch.cat(batch_cam_features, axis=0)
                    pos = pos[0]
            ...
            # Use env_state instead of images
            qpos = self.input_proj_robot_state(qpos)
            env_state = self.input_proj_env_state(env_state)
            transformer_input = torch.cat(
                [qpos, env_state], axis=1)  # seq length = 2
...
class CNNMLP(nn.Module):
    def __init__(self, backbones, state_dim, camera_names):
        ...
        if backbones is not None:
            ...
            self.mlp = mlp(input_dim=mlp_in_dim, hidden_dim=1024,
                           output_dim=14, hidden_depth=2)
        else:
...
def build_point_cloud(args):
    #point_cloud_model = PointNet()
    #point_cloud_model = MinkowskiPointNet()
    point_cloud_model = SparsePointNet()
    position_embedding = build_position_encoding(args)
    model = Joiner(point_cloud_model, position_embedding)
    return model
...
def build(args):
    ...
    #point_cloud_model = build_point_cloud(args)
    ...
    model = DETRVAE(
        ...
        use_point_clouds=args.use_point_clouds
    )
